...
public class FacebookDownloadView extends Composite {
	...
	private VerticalPanel mainPanelDownload;
	private FlexTable filesTable;// = new FlexTable();

	private ScrollPanel panelScroll;
	...
	final Button buttonFBDownloadPhotos = new Button(
			"Obtén las fotos de Facebook");

	public FacebookDownloadView(IntViews params) {
		// MAIN PANEL
		filesTable = new FlexTable();
		panelScroll = new ScrollPanel();
		...
		// si queremos pedir mas de un parametro se hace con "&". Ej:
		// https://graph.facebook.com/me/photos?fields=likes&access_token=xxxxx
	...
	void photosView(FacebookPhotos result) {
		...
		//ScrollPanel testPanel=new ScrollPanel();
		if (result != null && !result.getData().isEmpty()) {
			...
			for (FBDataPhoto a : result.getData()) {
				...
				// for(Images
				// ja:a.getImages()){link+="<a href=\""+ja.getSource()+"\"> "+ja.getHeight()+"</a>";}
				...
				String output = "<span> <a href=\"" + a.getLink()
						+ "\"><img src=\"" + a.getPicture() + "\" alt=\""
						+ a.getName() + " (" + a.getId()
						+ ")\"></a></span><br/>";
